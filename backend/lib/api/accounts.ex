defmodule Api.Accounts do
  ...
  def find_user_by_username_email(username, email) do
    query = from(u in User, where: u.username == ^username and u.email == ^email, select: u)
    Repo.all(query)
  end

  def find_workingtime(userID, id) do
    query = from(w in Workingtime, where: w.user == ^userID and w.id == ^id, select: w)
    Repo.all(query)
  end

  def find_workingtime_by_start_end(userID, start_date, end_date) do
    query = from(w in Workingtime,
     where: w.user == ^userID and w.start >=^start_date and w.end <= ^end_date,
     select: w)
    Repo.all(query)
  end

  def find_clock(userID) do
    query = from(c in Clock, where: c.user == ^userID, select: c)
    Repo.all(query)
  end

  def find_workingtime_by_user_id(userID) do
    query = from(w in Workingtime,
      where: w.user == ^userID ,
      select: w)
    Repo.all(query)
